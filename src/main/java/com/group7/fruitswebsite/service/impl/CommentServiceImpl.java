...
public class CommentServiceImpl implements CommentService {
	...
	@Override
	public ResponseEntity<ApiResponse> saveOne(DhCommentModel dhCommentModel,String username) {
		try {
			DhComment dhComment = objectMapper.readValue(objectMapper.writeValueAsString(dhCommentModel),
					DhComment.class);
			...
			if (currentProduct.isPresent()) {
				...
				return ApiResponseUtil.getCustomStatusWithMessage(Constants.ApiMessage.PRODUCT_ID_IS_NOT_DEFINED,
	...
	@Override
	public ResponseEntity<ApiResponse> getAll(Integer productId) {
		List<DhCommentDto> commentDtos = null;
		List<DhComment> comments = null;
		try {
			comments = commentRepository.findByDhProductId(productId);
			commentDtos = generateDtoToSendApi(comments);
			ApiResponse.ApiResponseResult apiResponseResult = ApiResponseUtil.mapResultWithOnlyData(commentDtos);
			return ApiResponseUtil.getBaseSuccessStatus(apiResponseResult);
		} catch (Exception ex) {
			log.error(String.format("Get all comment for product %s error "), ex);
			return ApiResponseUtil.getBaseFailureStatus();
		}
	}
	...
	public ResponseEntity<ApiResponse> getAllForProductWithPaging(int productId, int page, int size) {
		try {
			...
			ApiResponse.ApiResponseResult apiResponseResult = ApiResponseUtil.mapResultFromList(data, pageComments,
					totalCommentsForCurrentProduct, size);
			return ApiResponseUtil.getBaseSuccessStatus(apiResponseResult);
		} catch (Exception ex) {
