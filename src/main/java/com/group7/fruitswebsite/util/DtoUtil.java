...
public class DtoUtil {
	...
	public static DhUserProductDto getDtoFromUserProduct(DhUserProduct dhUserProduct, ObjectMapper objectMapper) {
		try {
			return objectMapper.readValue(objectMapper.writeValueAsString(dhUserProduct), DhUserProductDto.class);
		} catch (Exception e) {
	...
	public static DhUserAndRoleDto getDtoFromUserAndRole(DhUser dhUser, ObjectMapper objectMapper) {
		try {
			...
			DhUserAndRoleDto dhUserAndRoleDto = objectMapper.readValue(objectMapper.writeValueAsString(dhUserDto),
					DhUserAndRoleDto.class);
			for (DhRole dhRole : dhUser.getDhRoles()) {
				...
				if (dhRole.getName().equals(Constants.RoleName.ADMIN.getName())
						&& (StringUtils.isEmpty(dhUserAndRoleDto.getRole())
								|| dhUserAndRoleDto.getRole().equals(Constants.RoleName.USER.getName()))) {
					dhUserAndRoleDto.setRole(dhRole.getName());
					continue;
				}
				dhUserAndRoleDto.setRole(dhRole.getName());
	...
	public static DhProductDto getDtoFromProduct(DhProduct dhProduct, ObjectMapper objectMapper,
			ProductImageRepository productImageRepository) {
		try {
			DhProductDto productDto = objectMapper.readValue(objectMapper.writeValueAsString(dhProduct),
					DhProductDto.class);
			...
			if (!listCurrentProductImage.isEmpty()) {
				productDto.setProductImages(
						listCurrentProductImage.stream().map(DhProductImage::getPath).collect(Collectors.toList()));
				List<DhProductImageDto> listProductImageDtos = listCurrentProductImage.stream()
						.map(val -> getProductImageDtoFromProduct(val, objectMapper)).collect(Collectors.toList());
	...
	public static DhProductImageDto getProductImageDtoFromProduct(DhProductImage dhProductImage,
			ObjectMapper objectMapper) {
		try {
			DhProductImageDto productImageDto = objectMapper.readValue(objectMapper.writeValueAsString(dhProductImage),
					DhProductImageDto.class);
			productImageDto
					.setProductId(dhProductImage.getDhProduct() == null ? -1 : dhProductImage.getDhProduct().getId());
	...
	public static DhOrderDto getOrderDtoFromDhOrder(DhOrder dhOrder, ObjectMapper objectMapper,
		...
		try {
			...
			if (Objects.nonNull(orderProductRepository)) {
				...
				for (DhOrderProduct index : listAllProducts) {
					DhOrderProductDto dto = objectMapper.readValue(objectMapper.writeValueAsString(index),
							DhOrderProductDto.class);
